...
fn render_object(map: &serde_json::Map<String, serde_json::Value>, depth: usize) {
    ...
    if let Some(removed) = map.get("removed").and_then(serde_json::Value::as_bool) {
        println!(
            "{} {}",
...
fn render_pull(map: &serde_json::Map<String, serde_json::Value>) {
    ...
    if let Some(path) = map.get("logs_path").and_then(serde_json::Value::as_str) {
...
fn render_output(map: &serde_json::Map<String, serde_json::Value>) {
    ...
    if let Some(body) = map.get("text").and_then(serde_json::Value::as_str) {
    ...
    if let Some(bytes) = map.get("bytes").and_then(serde_json::Value::as_u64) {
...
fn render_row(value: &serde_json::Value) {
    ...
    if let Some(model) = map.get("last_model").and_then(serde_json::Value::as_str) {
        print!("  model={model}");
    }
    if let Some(count) = map.get("event_count").and_then(serde_json::Value::as_u64) {
        print!("  events={count}");
    }
    println!();
}
