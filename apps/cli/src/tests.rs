...
fn cli_parses_registry_sync_show_and_tagged_pull() {
    ...
    let show = Cli::try_parse_from(["takokit", "library", "show", "qwen3-tts:0.6b-base"])
        .expect("registry show");
    let pull = Cli::try_parse_from(["takokit", "pull", "whisper:small"]).expect("tagged pull");
...
#[test]
fn cli_parses_custom_model_and_voice_commands() {
    let custom = Cli::try_parse_from(["takokit", "custom-model", "add", "custom.toml"])
        .expect("custom model add");
    ...
    assert!(matches!(
          custom.command,
          Some(Command::CustomModel {
    command: CustomModelCommand::Add { manifest }
          }) if manifest == PathBuf::from("custom.toml")
      ));
    assert!(matches!(
          voice.command,
          Some(Command::Voice {
    command: VoiceCommand::Add {
        ...
        consent: true,
          ...
          }) if sample == PathBuf::from("sample.wav")
    && name == "Amaan"
