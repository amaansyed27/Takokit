...
use std::{fs, path::{Component, Path, PathBuf}, time::{SystemTime, UNIX_EPOCH}};
...
pub(crate) fn run_reset_command(store: &LocalStore, config: &RuntimeConfig, args: ResetArgs, json: bool) -> anyhow::Result<()> {
    let root = absolute_lexical(store.root())?;
    validate_global_root(&root)?;
    let project = args.project_data.as_ref().map(|path| resolve_project_tako(path)).transpose()?;
    ...
    let confirm = args.confirm.as_ref().ok_or_else(|| anyhow::anyhow!("full reset requires --confirm with the exact resolved Takokit home: {}", root.display()))?;
    if absolute_lexical(confirm)? != root {
        anyhow::bail!("reset acknowledgement does not exactly match resolved Takokit home {}", root.display());
    }

    if let Some(project) = project.as_ref() {
        let confirm = args.confirm_project_data.as_ref().ok_or_else(|| anyhow::anyhow!("project data requires separate --confirm-project-data with exact path {}", project.display()))?;
        if absolute_lexical(confirm)? != *project {
            anyhow::bail!("project acknowledgement does not exactly match {}", project.display());
        }
...
fn build_plan(root: &Path, project: Option<&Path>) -> ResetPlan {
    let categories = [
        ("models", "installed model snapshots and materialized artifacts"),
        ...
        ("manifests", "installed package manifests and ownership records"),
        ("voices", "instant voices, trained RVC projects, checkpoints, and package keys"),
    ...
    let targets = categories.into_iter().map(|(name, description)| {
        let path = root.join(name);
        ResetTarget { category: name.to_string(), exists: path.exists(), bytes: path_size(&path), path, action: format!("remove {description}") }
    }).collect();
    let project_target = project.map(|path| ResetTarget {
        category: "project-data".to_string(), exists: path.exists(), bytes: path_size(path), path: path.to_path_buf(), action: "remove this project's .tako sessions, files, and outputs only".to_string()
    });
    ResetPlan {
        takokit_home: root.to_path_buf(),
        installed_application: distribution::application_root(),
        installed_application_action: "preserve; Windows uninstall owns application files, Start Menu, and PATH".into(),
...
fn print_plan(plan: &ResetPlan, json: bool) -> anyhow::Result<()> {
    ...
    if let Some(path) = plan.installed_application.as_ref() { println!("  application       {} (preserved)", path.display()); }
    println!("  daemon            {}", plan.daemon_action);
    println!("  Windows PATH      {}", plan.path_action);
    println!();
    for target in &plan.targets {
        println!("  {:<12} {:>12}  {}  {}", target.category, format_bytes(target.bytes), if target.exists { "present" } else { "absent " }, target.path.display());
    }
...
fn validate_global_root(root: &Path) -> anyhow::Result<()> {
    ...
    if let Some(user) = std::env::var_os("USERPROFILE").map(PathBuf::from) {
        if absolute_lexical(&user)? == root { anyhow::bail!("Takokit home resolves to the user profile; refusing reset"); }
    }
    if let Some(install) = distribution::application_root() {
        if absolute_lexical(&install)? == root { anyhow::bail!("Takokit home resolves to the application installation root; refusing reset"); }
    }
...
fn validate_project_root(project: &Path, global_root: &Path) -> anyhow::Result<()> {
    ...
    if project.file_name().and_then(|name| name.to_str()) != Some(".tako") { anyhow::bail!("project reset target must resolve to a .tako directory"); }
    if project == global_root || project.starts_with(global_root) { anyhow::bail!("project .tako target must be separate from Takokit global storage"); }
    Ok(())
}

fn resolve_project_tako(path: &Path) -> anyhow::Result<PathBuf> {
    let absolute = absolute_lexical(path)?;
    Ok(if absolute.file_name().and_then(|name| name.to_str()) == Some(".tako") { absolute } else { absolute.join(".tako") })
}

fn reject_unsafe_path(path: &Path, label: &str) -> anyhow::Result<()> {
    if !path.is_absolute() { anyhow::bail!("{label} must resolve to an absolute path"); }
    let components = path.components().collect::<Vec<_>>();
    if components.len() <= 1 || components.iter().any(|component| matches!(component, Component::ParentDir)) {
        anyhow::bail!("{label} resolves to an unsafe path: {}", path.display());
    }
    #[cfg(windows)]
    if components.len() <= 2 { anyhow::bail!("{label} cannot be a drive root: {}", path.display()); }
    Ok(())
}

fn remove_confirmed_tree(path: &Path) -> anyhow::Result<()> {
    match fs::symlink_metadata(path) {
        Ok(metadata) if metadata.file_type().is_symlink() => anyhow::bail!("refusing to recursively delete symlink {}", path.display()),
...
fn absolute_lexical(path: &Path) -> anyhow::Result<PathBuf> {
    ...
    for component in absolute.components() {
        match component { Component::CurDir => {}, Component::ParentDir => { normalized.pop(); }, other => normalized.push(other.as_os_str()) }
    }
    Ok(normalized)
}

fn reset_journal_path() -> PathBuf {
    std::env::var_os("LOCALAPPDATA").map(PathBuf::from).unwrap_or_else(std::env::temp_dir).join("Takokit").join("reset-journal.json")
}
...
fn format_bytes(bytes: u64) -> String {
    ...
    let mut value = bytes as f64; let mut unit = 0;
    while value >= 1024.0 && unit + 1 < UNITS.len() { value /= 1024.0; unit += 1; }
    if unit == 0 { format!("{} {}", bytes, UNITS[unit]) } else { format!("{value:.2} {}", UNITS[unit]) }
}
