    ...
    for row in rows {
        ...
        if manifest.capabilities.stt {
            let Some(audio) = file else {
                row.run_result = Some(
                    "manual: provide --file <speech.wav> to run a real transcription smoke"
                        .to_string(),
                );
                continue;
            };
            ...
            .map_err(cli_error)
            {
                ...
                .and_then(|response| {
                    if response.text.trim().is_empty() {
                        Err(takokit_core::TakokitError::Execution(
                            "transcript was empty".to_string(),
                        ))
                    } else {
                        Ok(response)
        ...
        if manifest.capabilities.tts {
            ...
            let instruction = (row.model == "qwen3-tts-1.7b-voice-design").then(|| {
                "Warm, clear, confident narration with a natural pace.".to_string()
            });
