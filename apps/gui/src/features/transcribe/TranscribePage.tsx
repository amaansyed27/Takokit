...
export function TranscribePage({ runtime, onRefresh }: RouteComponentProps) {
  ...
  async function runTranscription() {
    ...
    setTranscript(null);
    try {
      const response = await transcribeSessionAudio({ model: selectedModel?.id, file_path: filePath });
      setTranscript(response.text);
      setNotice(`Transcript generated by ${response.model} and saved in this session.`);
    } catch (error) {
  ...
  return (
    <section className="page">
      <header className="page__header">
        <h1>Transcribe</h1>
        <p>Run local STT through the same Whisper path used by the CLI, TUI, and API.</p>
      </header>
      ...
      <Section title="Audio input" description="Use a local file path that the Takokit daemon can read.">
        <div className="upload-zone">
          ...
          <div className="field">
            ...
            />
            <small>Takokit reads the local file and saves the transcript in the active <code>.tako</code> session.</small>
          </div>
          <Button
            type="button"
            variant="primary"
