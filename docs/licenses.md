...
- `kokoro`: Takokit pins and verifies the v1.0 INT8 ONNX model and voice bundle from the `kokoro-onnx` release. The model weights are Apache-2.0 and `kokoro-onnx` is MIT. Its isolated JSON subprocess adapter uses the upstream phonemization stack. The macOS distribution also bundles the pinned eSpeak NG 1.52.0 native runtime (GPL-3.0-or-later plus component notices) because the upstream loader wheel is not relocatable there; its license texts and exact source URL/checksum ship beside the runtime. This does not link eSpeak into the Rust binaries.
