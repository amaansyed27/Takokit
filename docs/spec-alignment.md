...
- Short `tako` command alias for the same CLI and `~/.takokit` storage root
- Piper ONNX artifact/config loading scaffold
...
- `tako` is accepted as a short alias for the same local CLI, not a separate product or storage tree.
