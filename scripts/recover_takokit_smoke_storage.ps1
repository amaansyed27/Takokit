...
if ($owners.Count -gt 0) {
    ...
    foreach ($processId in $owners) {
        if (Test-TakokitDaemonProcess $processId) {
            Write-Host "Stopping Takokit daemon tree PID $processId..." -ForegroundColor Cyan
            Stop-TakokitProcessTree -ProcessId $processId
            continue
        }
...
Write-Host "The updated prefetch and smoke scripts use --direct and never use port 5050." -ForegroundColor Green
