...
fn rvc_inference_source_is_not_editable_installed() {
    ...
    assert!(!source.editable, "RVC source must not pull its Poetry dependency graph");
...
fn rvc_runtime_dependencies_do_not_compile_fairseq() {
    let spec = adapter_spec("rvc").expect("RVC inference adapter");
    assert!(spec.packages.iter().any(|item| item.starts_with("transformers")));
    assert!(spec.packages.contains(&"av==12.0.0"));
    assert!(spec.packages.iter().all(|item| !item.to_ascii_lowercase().contains("fairseq")));
}
