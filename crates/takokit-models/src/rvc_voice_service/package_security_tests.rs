use super::*;
use std::{collections::BTreeMap, fs, fs::File, io::{Read, Write}, path::Path};
...
fn export(service: &RvcVoiceService, project: &RvcVoiceProject, root: &Path, sign: bool) -> std::path::PathBuf {
    let package = root.join(if sign { "signed.takovoice" } else { "unsigned.takovoice" });
    service
        .export_package(
    ...
    for sign in [false, true] {
        ...
        let imported = service
            .import_package(ImportRvcPackageRequest {
                package,
                name: Some(if sign { "Signed import".into() } else { "Unsigned import".into() }),
                consent_affirmed: true,
...
fn altered_manifest_invalidates_signature() {
    ...
    let mut manifest: serde_json::Value = serde_json::from_slice(&entries["manifest.json"]).unwrap();
    manifest["voice_name"] = serde_json::json!("Altered voice");
    entries.insert("manifest.json".into(), serde_json::to_vec_pretty(&manifest).unwrap());
    rewrite(&package, &entries);
    let report = service.verify_package(&package).unwrap();
    assert_eq!(report.signature_valid, Some(false));
    assert!(report.errors.iter().any(|error| error.contains("signature")));
}
...
fn altered_checkpoint_is_rejected_by_sha256_and_size_metadata() {
    ...
    assert!(report.errors.iter().any(|error| error.contains("checkpoint.pth")));
}
...
fn altered_index_is_rejected() {
    ...
    let report = service.verify_package(&package).unwrap();
    assert!(!report.hashes_valid);
    assert!(report.errors.iter().any(|error| error.contains("model.index")));
}
...
fn invalid_signature_metadata_is_rejected() {
    ...
    let package = export(&service, &project, temp.path(), true);
    let mut entries = read_entries(&package);
    let mut signature: serde_json::Value = serde_json::from_slice(&entries["signature.json"]).unwrap();
    signature["signature_hex"] = serde_json::json!("00".repeat(64));
    entries.insert("signature.json".into(), serde_json::to_vec_pretty(&signature).unwrap());
    rewrite(&package, &entries);
    let report = service.verify_package(&package).unwrap();
    assert_eq!(report.signature_valid, Some(false));
}
...
fn manifest_path_traversal_is_reported_and_import_refuses_it() {
    ...
    rewrite(&package, &entries);
    let report = service.verify_package(&package).unwrap();
    assert!(report.errors.iter().any(|error| error.contains("unsafe manifest artifact path")));
    assert!(service.import_package(ImportRvcPackageRequest {
        package,
        name: None,
        consent_affirmed: true,
        consent_note: None,
...
fn package_file_count_bound_is_enforced() {
    ...
    let options = SimpleFileOptions::default();
    for index in 0..17 {
        writer.start_file(format!("entry-{index}"), options).unwrap();
...
fn export_excludes_training_dataset_by_default() {
    ...
    service.add_samples(&project.id.to_string(), AddRvcSamplesRequest { paths: vec![sample] }).unwrap();
    let package = export(&service, &project, temp.path(), false);
    ...
    assert!(!entries.keys().any(|name| name.contains("sample") || name.contains("dataset") || name.contains("training.wav")));
}
